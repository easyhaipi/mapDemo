...
/** 位置字典数组数组 包含  经纬度，距离和速度  */
@property (nonatomic, strong) NSMutableArray *locationDictM;
...
- (void)setupNavigationProperty
{
    ...
    UIBarButtonItem *listBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"足记历史"
                                                                         style:UIBarButtonItemStylePlain
                                                                        target:self
                                                                        action:@selector(trackList)];
    
    
    UIBarButtonItem *stopTrackButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"结束"
                                                                           style:UIBarButtonItemStylePlain
                                                                          target:self
                                                                          action:@selector(stopTrack)];
    // 导航栏右侧按钮
    self.navigationItem.rightBarButtonItems = @[listBarButtonItem,stopTrackButtonItem];
}
...
- (void)initBMLocationService
{
    ...
    NSLog(@"---更新位置频率---%f", [BMKLocationService getCurrentLocationDistanceFilter]);
...
-(void)trackList
{
    RecordListViewController *list = [[RecordListViewController alloc] init];
...
- (void)saveRoute
{
    ...
    NSString *path = [FileHelper filePathWithName:[formatter stringFromDate:date]];
    NSMutableDictionary *temdic = [[NSMutableDictionary alloc] init];
    [temdic setObject:self.locationDictM forKey:@"sportNodes"];
    [temdic writeToFile:path atomically:YES];
...
- (void)startTrailRouteWithUserLocation:(BMKUserLocation *)userLocation
{
    ...
    [dict setObject:@(userLocation.location.coordinate.latitude) forKey:@"latitude"];
    [dict setObject:@(userLocation.location.coordinate.longitude) forKey:@"longitude"];
    [dict setObject:@(self.sumDistance) forKey:@"distance"];
    [dict setObject:@(self.sumTime) forKey:@"time"];
    
    [self.locationDictM addObject:dict];
...
- (void)clean
{
    ...
    [self.locationDictM removeAllObjects];
...
-(NSMutableArray *)locationDictM
{
    if(_locationDictM == nil)
    {
        _locationDictM = [NSMutableArray array];
    }
    return _locationDictM;
}
